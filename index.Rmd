...
For **Density plots of columns of training data** see **Appendix**.<br>
Some of the columns are highly skewed, indicating presence of outliers. They are (with outlier values):
...
Their density plots are shown below (with outlier values). See Appendix for rest of the density plots.
...
The myTrain data was cross-validated 5 times on the optimal random forest, and from that, the out of sample error was calculated. I used cross-validation of K = 7 so that it isnt very high to introduce more variance, neither too low to introduce any bias. ntree = 400 was used because it had lesser prediction error on myTest, than default ntree=500. I also used parallel processing to reduce the time needed to build the model.
...
acc
```
The **out of sample error** can be calculated as **100 - acc** as percentage.<br>
```{r, echo=TRUE}
100 - acc
```
The **out of sample error** was found to be **0.6930462%**.

#### 3. Final Model OOB error rate = 0.58%<br>
...
#### 4. Accuracy on the whole training set = 99.89%<br>
```{r, echo=TRUE}
...
+ **training data having outliers (blue)**
+ **training data with outliers removed (pink)**
+ **myTrain (cyan)** (75% of cleaned training data)

```{r, echo=FALSE}
trn <- read.csv(file = "pml-training.csv", na.strings = c("","NA"))
NA_Colmns <- 100 * colSums(is.na(trn)) / (dim(trn)[1])
NA_Colmns <- NA_Colmns > 80
trn <- trn[, ! NA_Colmns]
drops <- c("X", "user_name", "raw_timestamp_part_1", "raw_timestamp_part_2",
           "cvtd_timestamp", "new_window", "num_window")
remv_colms <- colnames(trn) %in% drops
trn <- trn[, !remv_colms]



testing <- read.csv(file = "pml-testing.csv", na.strings = c("","NA"))
NA_Colmns <- 100 * colSums(is.na(testing)) / (dim(testing)[1])
NA_Colmns <- NA_Colmns > 80
testing <- testing[, ! NA_Colmns]
drops <- c("X", "user_name", "raw_timestamp_part_1", "raw_timestamp_part_2",
           "cvtd_timestamp", "new_window", "num_window", "problem_id")
remv_colms <- colnames(testing) %in% drops
testing <- testing[, !remv_colms]
# nearZeroVar(testing,saveMetrics=FALSE)
# dim(testing)



NAM <- colnames(training)
NAM_2 <- colnames(testing)
seqnc <- seq(1,52)
par(mfrow = c(2,3))
barplot(table(trn[,53]), col = "#7F7FFF", main=NAM[53])
barplot(table(training[,53]), col = "#FD7F82", main=NAM[53])
barplot(table(myTrain[,53]), col = "#00FFFF", main=NAM[53])
# barplot(table(predict(modelFit, testing)), col = "orange", main=NAM_2[53])
...
for(i in seqnc){
        ...
        dens <- density(trn[,i])
        plot(dens, main=NAM[i])
        polygon(dens, col="#7F7FFF", border="black")
        
        
        dens <- density(training[,i])
        plot(dens, main=NAM[i])
        polygon(dens, col="#FD7F82", border="black")
        
        
        dens <- density(myTrain[,i])
        plot(dens, main=NAM[i])
        polygon(dens, col="#00FFFF", border="black")
        #07FDFF
        
        #dens <- density(testing[,i])
        #plot(dens, main=NAM_2[i])
        #polygon(dens, col="orange", border="black")
        
        # abline(v = mean(training[,i]), col = "black", lwd = 2)
}
```
...
cor_trn <- training[,-53]
cor_trn <- cor(cor_trn)
ncolms <- dim(cor_trn)[2]

NAM <- colnames(cor_trn)
...
for(i in 2:ncolms){
        J <- i-1
        for(JJ in 1:J){
                if(   abs(cor_trn[i,JJ]) >= 0.7   ){
                        ny[ctr,1] <- cor_trn[i,JJ]
                        ny[ctr,2] <- NAM[i]
                        ny[ctr,3] <- NAM[JJ]
                        ctr <- ctr + 1
                }
...
colnames(ny) <- c("corr coeff","Colm 1","Colm 2")
ny <- ny[order(-ny[,"corr coeff"]),]
rownames(ny) <- NULL
#ny  #print ny to view the table
```